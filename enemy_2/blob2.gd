...
func handle_on_floor(_delta):
	...
	$RayCast2D.set_target_position(to_local(player_position.value))
...
func _on_vission_area_body_entered(body):
	if !body.is_in_group("player"):
		return
...
func _on_vission_area_body_exited(body):
